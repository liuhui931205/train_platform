...
class ProcessLabelRemoteHandler(BaseProcessLabel):
    def __init__(self):
        ...
        self.temp_dir = GlobalVar.check_dir.value + "_temp"
        ...
        self.remote_cut_queue = manager.Queue()
    ...
    def start_process(self, version, types, task_id, status, color_info):
        ...
        name = time.strftime("lane-aug-%Y%m%d", time.localtime())
        ...
        try:
            ...
            if not os.path.exists(self.src_dir):
                    ...
                    for _image in origin_list:
                        ...
                        if not os.path.exists(origin_image):
                            ...
                            current_app.logger.info(
                                "package:{}, file missing:[{}]=>[{}]".format(str(i), origin_name, _image))
                            continue
                ...
                for process in all_processes:
                    process.start()
                    print(str(process.pid) + ", start")

                for process in all_processes:
                    process.join()
                    print(str(process.pid) + ", join")
            ...
            with open(_csv_file, "r") as f:
                ...
                while line_str:
                    ...
                    if not os.path.exists(os.path.join(root_dir, label)):
                        line_str = f.readline()
                        current_app.logger.info(
                            "package:{}, label missing:[{}]=>[{}]".format(package_index, image, label))
                        continue

                    if not os.path.exists(os.path.join(root_dir, image)):
                        line_str = f.readline()
                        current_app.logger.info(
                            "package:{}, image missing:[{}]=>[{}]".format(package_index, image, label))
                        continue
    ...
    def do_work(self):
        if self.remote_cut_queue.empty():
            return

        while not self.remote_cut_queue.empty():
            task = self.remote_cut_queue.get()
            ...
            img = cv2.imread(_src)
            if img is None:
                current_app.logger.error("image is none[{}/{}]".format(task.package_index, _src))
                continue
            width = img.shape[1]
            height = img.shape[0]
            ...
            if width == 2448 and height == 2048:
                crop_img = img[height // 2:height, 0:width]
            else:
    ...
    def transform(self, dicts):
        ...
        while not self.remote_process_queue.empty():
            ...
            if img is None:
                current_app.logger.error("image is none[{}/{}]".format(task.package_index, image_path))
                continue
                ...
                while line_str:
                    ...
                    if not os.path.exists(image_path):
                        if os.path.exists(label_path):
                            current_app.logger.info(
                                "package:{}, not match:[{}] not exist=>[{}] exist".format(package_index, image, label))
                            if os.path.exists(label_path):
                                os.remove(label_path)
                        else:
                            current_app.logger.info(
                                "package:{}, file missing:[{}]=>[{}]".format(package_index, image, label))
                    else:
                        if not os.path.exists(label_path):
                            current_app.logger.info(
                                "package:{}, not match:[{}] exist=>[{}] not exist".format(package_index, image, label))
                            if os.path.exists(image_path):
                                os.remove(image_path)
