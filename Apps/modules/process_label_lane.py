...
class ProcessLabelLaneHandler(BaseProcessLabel):
    def __init__(self):
        ...
        try:
            ...
            if not os.path.exists(self.src_dir):
                ...
                for process in all_processes:
                    process.start()
                    current_app.logger.info(str(process.pid) + ", start")

                for process in all_processes:
                    process.join()
                    current_app.logger.info(str(process.pid) + ", join")
    ...
    def do_work(self):
        ...
        while not self.full_cut_queue.empty():
            ...
            if crop_w == 2448 and crop_h == 2048:
                crop_img = crop_img[crop_h // 2:crop_h, 0:crop_w]
    ...
    def clean_dir(self, _csv_file):
        ...
        if _csv_file:
            ...
            for dir in self.dir_path:
                ...
                #         continue
                #     if file_id.endswith("xlsx"):
                #         file_delete = os.path.join(dir, file_id.decode("UTF-8"))
                #         os.remove("\"" + file_delete + "\"")
                #
                #     if file_id not in image_list:
                #         file_delete = os.path.join(dir, file_id)
