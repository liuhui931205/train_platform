...
class EvalData(object):
    ...
    def __init__(self,
        ...
        self.upsample_mod.bind(data_shapes=[('data', (self.batch_size, self.num_classes, self.h, self.w))],
                               label_shapes=None,
                               for_training=False,
                               force_rebind=True)
        if self.add_conf:
            self.confidence_mod.bind(data_shapes=[('data', (1, int(self.num_classes), self.h, self.w))],
                                     label_shapes=None,
                                     for_training=False,
                                     force_rebind=True)
    ...
    def run(self, img_file, dest_path):
        ...
        if self.save_result_or_not:
            self.save_result(img_file, dest_path, pred_label[0], confidence_outputs)
    ...
    def read_image(self, img_file):
        ...
        if self.use_half_image:
            ...
            h = height // 2
            # img = img.resize((w, h))
            img = np.array(Image.fromarray(bottom_half.astype(np.uint8, copy=False)).resize((w, h), Image.NEAREST))
    ...
    def save_result(self, img_file, dest_path, pred_label, confidence_outputs):
...
def do_seg(image_type, gpu_id, task_queue, model_path, count, cls, label_data, lock):

    net = EvalData(model_path,
                   gpu_id,
                   cls,
                   image_type,
                   use_half_image=True,
...
if __name__ == '__main__':
    ...
    for i in range(gpu_count * single_count):
        ...
        t = multiprocessing.Process(target=do_seg,
                                    args=(gpu_id, output_confidence, task_queue, model_path, count, cls, label_data,
                                          label_map_data, lock, update_queue))
