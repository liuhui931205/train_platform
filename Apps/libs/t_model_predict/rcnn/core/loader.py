...
from Apps.libs.t_model_predict.rcnn.config import config
from Apps.libs.t_model_predict.rcnn.io.image import tensor_vstack
from Apps.libs.t_model_predict.rcnn.io.rpn import get_rpn_testbatch, get_rpn_batch, assign_anchor_fpn
from Apps.libs.t_model_predict.rcnn.io.rcnn import get_rcnn_testbatch, get_fpn_rcnn_testbatch, get_fpn_maskrcnn_batch
...
class AnchorLoaderFPN(mx.io.DataIter):
    ...
    def get_batch(self):
        ...
        for islice in slices:
            ...
            # data : {data, im_info}, label : {gt_boxes}
            data_list.append(data)
            label_list.append(label)

        # pad data first and then assign anchor (read label)
        data_tensor = tensor_vstack([batch['data'] for batch in data_list])
        # note: as using random scale, the data may be different size, tensor_vstack normalize them
