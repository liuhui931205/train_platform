...
class KuandengSign(IMDB):
    def __init__(self, image_set, root_path, dataset_path):
        ...
        :return: imdb object
        """
        super(KuandengSign, self).__init__('kuandeng', image_set, root_path, dataset_path)
        self.image_set = image_set
        self.root_path = root_path
        self.data_path = dataset_path

        self.classes = ['__background__', 'sign', "light"]
        self.class_id = [0, 1, 2]

        # filter too small objects
        self.min_bbox_area = 400.0
        ...
        for c in range(1, len(self.class_id)):
            ...
            for id in ids:
                ...
                if x_max - x_min <= 1 or y_max - y_min <= 1:
                    continue
                if (x_max - x_min) * (y_max - y_min) < self.min_bbox_area:
                    continue
