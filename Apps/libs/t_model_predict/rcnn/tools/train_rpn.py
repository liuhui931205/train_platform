...
def train_rpn(network, dataset, image_set, root_path, dataset_path,
    ...
    # set up logger
    logging_format = '%(levelname)s: %(asctime)s: %(filename)s:%(lineno)d * %(thread)d %(message)s'
    ...
    logging.info('providing maximum shape: (%s, %s)' % (str(max_data_shape), str(max_label_shape)))

    # infer shape
    data_shape_dict = dict(train_data.provide_data + train_data.provide_label)
    logging.info("provide_data:%s provide_label:%s" % (str(train_data.provide_data), str(train_data.provide_label)))
