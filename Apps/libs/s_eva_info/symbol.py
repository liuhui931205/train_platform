...
def conv(data, name, filters, kernel=3, stride=1, dilate=1, pad=-1,
         groups=1, no_bias=False, workspace=-1, cudnn_off=False):
    """
        convolution wrapper
    """
    if kernel == 1:
...
def bn(data, name, eps=1.001e-5, fix_gamma=False, use_global_stats=None, cudnn_off=False):
    """
...
def softmax_out(data, label=None, grad_scale=1.0, multi_output=False, name='softmax'):
    """
        softmaxout wrapper
    """
    if multi_output:
        return mx.sym.SoftmaxOutput(data, label=label, name=name,
